...
from agent.planning.latent_mppi_search import LatentMPPIPlanner
from agent.MapProcess import GlobalNavField
...
class BehaviorSystem(MapProcesser):
    # ====================================================
    # 静态共享变量：确保所有智能体共享一个 MPPI 规划器，避免显存溢出
    # ====================================================
    _shared_mppi_planner = None
    _mppi_horizon = 10
    _models_loaded = False

    def __init__(self, agent):
        self.agent = agent
        # 从配置中读取是否启用 MPPI (默认为 False)
        self.use_latent_mppi = getattr(self.agent, 'use_latent_mppi', False)
        # 为当前智能体初始化全局拓扑导航对象
        self.global_nav = GlobalNavField()
        # 懒加载：只有在启用了 MPPI 且尚未加载模型时才初始化
        if self.use_latent_mppi and not BehaviorSystem._models_loaded:
            BehaviorSystem._init_shared_mppi()

    # ====================================================
    #  属性桥接 (Property Bridging)
    # ====================================================
    @property
    ...
    @classmethod
    def _init_shared_mppi(cls):
        """初始化共享的 MPPI 组件 (无需 VQ-VAE / ActionVAE)"""
        print(">>> [BehaviorSystem] 正在初始化全局共享的 MPPI 组件...")
        device = torch.device('cpu')
        POS_LIMIT = 500.0
        ANG_LIMIT = np.pi
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

        # 1. 加载 ForwardPredictor (新 ViT+AdaLN 架构)
        fwd_path = os.path.join(base_dir, "models/predictors/forward_model.pth")
        if not os.path.exists(fwd_path):
            print(f"  ❌ Forward Model 未找到: {fwd_path}，无法启用 MPPI")
            cls._models_loaded = True
            return

        # 从 checkpoint 推断 horizon
        fwd_ckpt = torch.load(fwd_path, map_location=device)
        horizon = fwd_ckpt.get('horizon', 10)
        # Fallback: 尝试从 causal_mask 推断
        if 'causal_mask' in fwd_ckpt:
            horizon = fwd_ckpt['causal_mask'].shape[0]

        cls._mppi_horizon = horizon
        ...
        # 2. 实例化 MPPI Planner
        cls._shared_mppi_planner = LatentMPPIPlanner(
        ...
        cls._models_loaded = True
        print(f">>> [BehaviorSystem] MPPI Planner 已激活 (horizon={horizon}, samples=64)")
    ...
    def task_allocate_model(self, obs_d=None):
        """
        高层任务分配与规划模型 (MPPI 采样 → 加权融合最优 action)

        obs_d: 底层 SAC 4-key Dict {lidar_2d, goal_dir, history_goal, dynamics}
        返回: 最优 action 向量 (5-dim)
        """
        if not getattr(self.agent, 'use_latent_mppi', False) or self._shared_mppi_planner is None or obs_d is None:
            # 无 MPPI 时存储零 action
            self.agent.mppi_action = np.zeros(5, dtype=np.float32)
            return self.agent.mppi_action

        # 提取当前观测
        lidar_2d = np.array(obs_d['lidar_2d'], dtype=np.float32)        # (5, 36)
        goal_dir = np.array(obs_d['goal_dir'], dtype=np.float32)        # (2,)
        dynamics = np.array(obs_d['dynamics'], dtype=np.float32)        # (2,)

        # 构建多帧观测 (从 agent buffer)
        from sim.obs_utils import build_multi_frame_obs, init_agent_buffers
        init_agent_buffers(self.agent)
        past_goals = [self.agent.goal_history[i] for i in range(2)]
        past_lidars = [self.agent.lidar_history[i] for i in range(2)]

        # 计算当前 goal_mask
        from sim.obs_utils import goal_to_lidar_mask
        dx = self.agent.p_pos[0] - self.agent.position[0]
        dy = self.agent.p_pos[1] - self.agent.position[1]
        rou = np.hypot(dx, dy)
        target_angle = np.arctan2(dy, dx)
        etheta = _angle_diff(self.agent.theta, target_angle)
        cur_goal_mask = goal_to_lidar_mask(rou, etheta)

        obs_frames = build_multi_frame_obs(past_goals, past_lidars, lidar_2d, cur_goal_mask)
        # (3, 36, 6)

        # MPPI 搜索需要 global map 信息
        if self.agent.down_sampled_map is not None and self.agent.grid_map is not None:
            ...
            self._shared_mppi_planner.update_global_map(
            ...
            best_action = self._shared_mppi_planner.search_best_action(
                obs_frames, dynamics, goal_dir, agent_global_info
            )
        else:
            best_action = self._shared_mppi_planner.search_best_action(
                obs_frames, dynamics, goal_dir, None
            )

        # 存储 MPPI 选中的 action 到 agent
        self.agent.mppi_action = best_action

        # 更新 agent buffers (用于下一帧的历史)
        from sim.obs_utils import update_agent_buffers
        update_agent_buffers(self.agent, cur_goal_mask, lidar_2d)

        return best_action

    def get_mppi_action(self, obs_d):
        """
        MPPI 规划 → 返回 5-dim action。

        返回: action (5,) numpy array
        """
        best_action = self.task_allocate_model(obs_d)

        # 简化输出：每 N 步打印一次
        if self.agent.k % 100 == 0:
            print(f"[MPPI] Agent {self.agent.id:2d} → "
                  f"action=[{best_action[0]:+.2f} {best_action[1]:+.2f} "
                  f"{best_action[2]:+.2f} {best_action[3]:+.2f} {best_action[4]:+.2f}]")
